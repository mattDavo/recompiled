...
With his C++ background, Dave talks about C++'s influence on the design of Swift. Swift and C++ are the only two widely used languages that use value types beyond basic literals for example with arrays in Swift and vectors in C++. However, the drawback of this in C++ is seen when passing large value types into functions, as it will directly worsen the performance as the size of the function parameters increases. This is typically combatted by passing the variable as a constant reference. For example:
