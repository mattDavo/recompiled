...
From the [MDN Docs](https://developer.mozilla.org/en-US/docs/Web/CSS/Using_CSS_custom_properties) CSS custom properties (also known as CSS variables and cascading variables) are "entities defined by CSS authors that contain specific values to be reused throughout a document." They are super useful for providing consistent styling across a large site. You will often see them applied to the `:root` object but there are other ways we can use them cleverly for our benefit. Once a css variable is defined in the styling of an element, it is available in all of children elements.
